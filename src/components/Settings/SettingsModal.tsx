...
export default function SettingsModal({ isOpen, onClose, initialTab = 'visual', onExitToHome }: SettingsModalProps) {
      ...
      <div className="w-full max-w-5xl h-[85vh] bg-zinc-950 border-2 border-amber-500/50 rounded-2xl shadow-[0_0_50px_rgba(245,158,11,0.2)] flex flex-col overflow-hidden relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 p-8 overflow-y-auto no-scrollbar bg-zinc-950/50">
            {activeTab === 'visual' && <VisualSettings settings={settings} />}
            {activeTab === 'prompt' && <PromptSettings settings={settings} />}
            {activeTab === 'api' && <ApiSettings settings={settings} />}
...
function VisualSettings({ settings }: { settings: GameSettings }) {
  return (
    <div className="space-y-8">
      <Section title="显示设置">
        <div className="grid grid-cols-2 gap-4">
          <SettingItem label="字体大小">
            <select 
              defaultValue={settings.visual.fontSize}
          ...
          <SettingItem label="界面缩放">
            <select 
              defaultValue={settings.visual.scale}
              className="w-full bg-zinc-900 border border-zinc-700 rounded-lg p-2 text-zinc-200 focus:border-amber-500 outline-none"
            >
              <option>100%</option>
              <option>125%</option>
      ...
      <Section title="视觉效果">
        <div className="space-y-4">
          <ToggleItem label="启用动态背景" description="在背景中显示微妙的粒子效果" defaultChecked={settings.visual.dynamicBackground} />
          <ToggleItem label="启用文字打字机效果" description="消息逐字显示" defaultChecked={settings.visual.typingEffect} />
          <ToggleItem label="启用界面光晕" description="为边框和重要元素添加发光效果" defaultChecked={settings.visual.uiGlow} />
        </div>
      </Section>
    </div>
  );
...
function PromptSettings({ settings }: { settings: GameSettings }) {
  const [activeAI, setActiveAI] = useState<'actionCollector' | 'mainStory' | 'stateProcessor'>('mainStory');

  const getAIPrompt = (type: typeof activeAI) => settings.ai[type].prompt;

  return (
    <div className="space-y-6 h-full flex flex-col">
      <div className="flex justify-between items-center">
        <h3 className="text-zinc-200 font-bold">AI 提示词配置</h3>
        <button className="flex items-center gap-2 text-xs text-amber-500 hover:text-amber-400 transition-colors">
      ...
      <div className="flex gap-2 p-1 bg-zinc-900 rounded-lg border border-zinc-800">
        <button
          onClick={() => setActiveAI('actionCollector')}
          className={`flex-1 py-2 px-3 rounded-md text-xs font-medium transition-all flex items-center justify-center gap-2
            ${activeAI === 'actionCollector' ? 'bg-zinc-800 text-amber-500 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
        >
          <Cpu size={14} /> 行动收集 AI
        </button>
        <button
          onClick={() => setActiveAI('mainStory')}
          className={`flex-1 py-2 px-3 rounded-md text-xs font-medium transition-all flex items-center justify-center gap-2
            ${activeAI === 'mainStory' ? 'bg-zinc-800 text-amber-500 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
        >
          <BookOpen size={14} /> 主剧情 AI
        </button>
        <button
          onClick={() => setActiveAI('stateProcessor')}
          className={`flex-1 py-2 px-3 rounded-md text-xs font-medium transition-all flex items-center justify-center gap-2
            ${activeAI === 'stateProcessor' ? 'bg-zinc-800 text-amber-500 shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}
        >
          <Calculator size={14} /> 数据处理 AI
        </button>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-4">
        <SettingItem label="温度 (Temperature)">
            ...
            min="0"
            max="2"
            defaultValue={getAIPrompt(activeAI).temperature}
        ...
        <div className="flex-1 flex flex-col gap-2">
          ...
          <textarea 
            ...
            defaultValue={getAIPrompt(activeAI).systemPrompt}
          />
        </div>
      </div>
      
      <div className="flex justify-end pt-4 border-t border-zinc-800">
        <button className="flex items-center gap-2 px-6 py-2 bg-amber-600 hover:bg-amber-500 text-zinc-950 rounded-lg font-bold transition-colors shadow-lg shadow-amber-900/20">
          <Save size={16} /> 保存配置
        </button>
      </div>
    </div>
...
function ApiSettings({ settings }: { settings: GameSettings }) {
  const [activeAI, setActiveAI] = useState<'actionCollector' | 'mainStory' | 'stateProcessor'>('mainStory');

  const getAIConnection = (type: typeof activeAI) => settings.ai[type].connection;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <SettingItem label="默认 API Endpoint">
            <div className="relative">
              ...
              <input 
                type="text" 
                defaultValue={settings.ai.defaultEndpoint}
          ...
          <SettingItem label="默认 API Key">
            <div className="relative">
              ...
              <input 
                type="password" 
                defaultValue={settings.ai.defaultApiKey}
                className="w-full bg-zinc-950 border border-zinc-700 rounded-lg py-2 pl-9 pr-2 text-zinc-200 focus:border-amber-500 outline-none font-mono text-xs"
                placeholder="sk-..."
              />
            </div>
      ...
      <div className="flex-1 flex flex-col gap-4">
        ...
        <div className="flex-1 bg-zinc-900/30 rounded-xl border border-zinc-800 p-4 space-y-4">
           <div className="flex items-end gap-2">
            <div className="flex-1">
              <SettingItem label="模型选择 (Model)">
                <div className="relative">
                  ...
                  <input 
                    type="text" 
                    defaultValue={getAIConnection(activeAI).model}
                    className="w-full bg-zinc-950 border border-zinc-700 rounded-lg py-2 pl-9 pr-2 text-zinc-200 focus:border-amber-500 outline-none font-mono text-xs"
                    placeholder="gpt-4o"
                  />
                </div>
            ...
            <button className="h-[34px] px-3 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 border border-zinc-700 rounded-lg flex items-center gap-2 text-xs transition-colors whitespace-nowrap mb-[1px]">
              <Download size={14} />
              获取模型列表
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 pt-2 border-t border-zinc-800/50">
            <SettingItem label="自定义 Endpoint (留空则使用全局默认)">
              <div className="relative">
                ...
                <input 
                  type="text" 
                  defaultValue={getAIConnection(activeAI).endpoint}
                  ...
                  placeholder={settings.ai.defaultEndpoint || "https://api.openai.com/v1"}
                />
              </div>
            </SettingItem>
            <SettingItem label="自定义 API Key (留空则使用全局默认)">
              <div className="relative">
                ...
                <input 
                  type="password" 
                  defaultValue={getAIConnection(activeAI).apiKey}
                  className="w-full bg-zinc-950 border border-zinc-700 rounded-lg py-2 pl-9 pr-2 text-zinc-200 focus:border-amber-500 outline-none font-mono text-xs placeholder:text-zinc-700"
      ...
      <Section title="数据管理">
        <div className="grid grid-cols-1 gap-4">
          <div className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
            ...
            <button className="px-4 py-2 border border-zinc-700 hover:bg-zinc-800 text-zinc-300 rounded-lg text-sm transition-colors">
              导出
            </button>
          </div>
          ...
          <div className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
            ...
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-red-900/30 hover:bg-red-900/50 border border-red-900/50 text-red-400 rounded-lg text-sm transition-colors"
            >
              <Trash2 size={16} /> 清除
            </button>
          </div>
